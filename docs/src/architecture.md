...
* **扁平化目录树 (Arena Tree)：** 抛弃容易死锁的嵌套指针锁，使用 `DashMap` 实现，采用 **读无锁并发、写 Actor 串行** 的读写分离模型：读操作直接并发查询 `DashMap` 分片，写操作统一由 `NameSystemActor` 单线程处理，彻底消除写锁竞争和死锁可能。
...
4. **容错与状态提交：** DataNode 落盘前必须强制进行 Checksum 二次校验。若中途节点宕机，Client 向 NameNode 上报故障，由 NameNode 升级版本号并返回新管线，Client 联系幸存节点执行长度对齐后降级续传，写完后调用 `CompleteFile` 正式闭环。

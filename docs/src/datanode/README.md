...
8. **无死锁流水线 (Deadlock-Free Pipeline)：** 采用多任务与 `mpsc` 通道解耦读写路径，通过严格背压策略（`try_send/send_timeout`、并发限流、超时回退），彻底规避环形死锁。
...
* **全双工与防死锁：** Pipeline 涉及上游推数据、下游推数据、本地写磁盘、下游回传 ACK、向上游回传 ACK 五个并发动作。若将每个动作拆分为独立任务并用有界 `mpsc` 通道连接，且发送端使用无超时的 `.await`，当下游处理缓慢时所有任务会相互等待，形成 **环形死锁**。以下是必须遵守的方案：
  * **优先单任务轮询：** 在一个异步任务内用 `tokio::select!` 同时轮询所有事件，避免跨任务通道依赖。
  * **必须拆分任务时：**
    * 所有跨任务发送必须使用 `try_send` 或 `send_timeout`，禁止无超时的 `.await`。
...
- [ ] **Phase 5：容错指令执行与主动扫描**
  - 解析 Heartbeat 返回的 `Command`，通过高优通道解耦，异步执行删除/复制动作。

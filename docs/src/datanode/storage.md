...
3. **Finalized (已完成)：**
   DataNode 将 `.data` 和 `.meta` 文件从 `/rbw/` 原子重命名（Atomic Rename）移入 `/current/subdir_xx/` 目录。此时该块正式处于稳定状态。
4. **Deleted (已删除)：**
...
RDFS 必须容忍磁盘发生静默错误（位翻转），校验和的计算和比对策略遵循 **“边缘计算（卸载 CPU 压力）”** 原则：

* **写入流 (Write Pipeline)：** Client 负责计算每个 64KB Chunk 的 CRC32 并发给 DataNode。DataNode 只负责将收到的 Chunk 存入 `.data`，将收到的 Checksum 追加存入 `.meta`，**不消耗 CPU 去做重复验证**。
* **读取流 (Read Pipeline)：** DataNode 响应 Read 请求时，只需无脑地将 `.data` 和 `.meta` 里的校验和打包成 gRPC Stream 发给 Client。**由 Client 在内存中比对 CRC32**。若 Client 发现不一致，它会主动废弃数据并向 NameNode 举报坏块。
* **主动防御 (Block Scanner)：** DataNode 的 CPU 只有在系统空闲时，才会通过后台运行的 `Block Scanner`（见 `recovery.md`）主动去读磁盘算 CRC，进行静默损坏的排查。
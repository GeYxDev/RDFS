...
3. **Finalized (已完成)：**
   ...
   ⚠️ 重命名铁律 (Rename Order)：必须先重命名 `.meta` 文件，再重命名 `.data` 文件。由于两个独立的 `rename` 系统调用无法做到绝对原子化，如果在此期间发生掉电，重启后 DataNode 扫描 `/current/` 目录时，可能会看到一个孤立的 `.data` 文件而没有对应的 `.meta`。遇到这种情况，DataNode 应当直接将该没有元数据的 `.data` 视为损坏或垃圾块进行安全清理。
4. **Deleted (已删除)：**

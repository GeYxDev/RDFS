...
### 4.2 调试 `tokio` 异步代码的技巧
分布式存储重度依赖 `tokio`。如果你发现系统卡死，很可能是某个管道 (Channel) 阻塞或遇到了死锁。
* 建议在调试时启用 `tokio-console` 支持（需在 `Cargo.toml` 中加入对应依赖并开启 `tokio_unstable` 标志），它能让你像任务管理器一样实时查看所有异步任务的状态。
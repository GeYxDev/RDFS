...
* **分段锁并发哈希表 (Sharded Lock)：** 放弃嵌套的 `Arc<RwLock<T>>` 树状结构，核心命名空间 `NamespaceMap` 强制使用 `DashMap`。通过逻辑 Inode ID 寻址，将高并发锁竞争物理隔离到不同的 Shard 中。
* **无锁化原子递增 (Lock-free Atomics)：** 诸如 `next_inode_id`、`next_block_id` 等高频计数器，全面使用 `std::sync::atomic::AtomicU64`，根除不必要的锁排序死锁风险。
...
- [ ] **Phase 1：高并发内存模型建立**
  - 定义 `Inode` 结构，引入 `DashMap` 与 `AtomicU64`，支持多级目录的并发读写。

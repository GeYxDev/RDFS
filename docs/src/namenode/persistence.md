...
> **注意：** SNN 在合并 FsImage 与 EditLog 时，仅关注元数据树的变化（创建/删除/重命名等），忽略 `AllocateBlock` 操作，因为该操作仅记录了 Block ID 分配，不包含物理位置信息，而 FsImage 本身也不存储 Block 的物理位置。合并后的新 FsImage 仍不含任何 Block 位置信息，这些信息会在 NameNode 重启后的安全模式中通过 DataNode 的 `BlockReport` 重建。

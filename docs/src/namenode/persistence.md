...
* **写入模型：**
  ...
  * 为避免与 Tokio 默认的 blocking 线程池争抢资源，EditLog 的刷盘操作应使用独立的专用线程池，推荐大小等于磁盘设备数（通常 1-2）。
    * 可使用 `rayon::ThreadPool` 或为 NameNode 配置独立的 `tokio::runtime::Builder::max_blocking_threads`。
    * 当专用线程池任务队列满时，新请求立即返回 `RESOURCE_EXHAUSTED` 错误，阻止 Actor 循环阻塞。
  * 可提供 `rdfs.namenode.editlog.threads`（默认 2）控制专用线程池大小。
* **批量提交：**
  * Actor 将连续收到的 N 个 Command（默认 50 个）或累积等待不超过 20ms 后，一次性序列化为一批记录并提交刷盘。
  * 批量刷盘完成后，Actor 再顺序将每个 Command 应用到 DashMap 并响应 Client。
  * 此机制大幅减少 `spawn_blocking` 调用次数，降低线程池饱和风险。若批量刷盘超时（5 秒），NameNode 应主动崩溃并记录错误日志，因为磁盘已处于不可用状态。
...
> **注意：** SNN 在合并 FsImage 与 EditLog 时，会处理所有修改元数据树的日志，包括 `AllocateBlock`（因为该操作会向文件的 Block 列表追加 Block ID）。合并后的 FsImage 中仅包含 Block ID 列表，不包含 Block 的物理位置信息。物理位置信息完全依赖 DataNode 的 `BlockReport` 在 NameNode 启动时重建。因此，SNN 合并时忽略的是物理位置有关的操作，而非 `AllocateBlock` 本身。

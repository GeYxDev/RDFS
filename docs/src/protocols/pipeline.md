...
2. **申请数据块与管线 (Allocate)：**
   Client 准备写入 Block 1，向 NameNode 发送 `AllocateBlock`。NameNode 生成一个全局唯一的 `block_id`、一个初始世代版本号 `gen_stamp`，并根据负载均衡策略挑选 3 个健康的 DataNode（假设为 DN1, DN2, DN3）。
3. **流式推送与管线建立 (Stream Pipeline)：**
   Client 与最近的 **DN1** 建立单向 gRPC Stream，利用 `oneof` 特性进行高效传输。
    * **首包 (WriteHeader)：** Client 发送元数据，携带目标管线 `[DN2, DN3]`。DN1 收到后，主动与 DN2 建流，并将目标剥离为 `[DN3]` 转发；DN2 再与 DN3 建流。
    * **后续包 (WriteChunk)：** Client 将 64MB 的数据切分为 64KB 的 Chunk，附带 `checksum` 持续推入。DN1 收到 Chunk 边落盘、边向 DN2 转发；DN2 边落盘、边向 DN3 转发。

...
* **进入条件：** NameNode 处理 `ReportFailedBlock` 或启动租约恢复时，必须原子检查该 Block 是否已被标记为 `UnderRecovery`，若是则拒绝本次恢复请求（返回 `RESOURCE_EXHAUSTED` 错误），由请求方稍后重试。
* **状态行为：**
...
> **接口统一说明：** `CompleteBlockRecovery`（Client 调用）和 `CommitBlockRecovery`（DataNode 调用）最终都会触发 NameNode 的同一处理逻辑：更新 Block 的 `gen_stamp` 和 `final_length`，并清除 `UnderRecovery` 标记。NameNode 应根据调用方身份（`client_id` 或 `primary_node_id`）记录审计日志，但执行结果无差别。

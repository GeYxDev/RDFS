...
**核心结构**：将逻辑上的 Block 与物理上的 DataNode 绑定，客户端拿到该结构后可知去哪里读写数据。
...
* **场景：** 客户端拿到 `AllocateBlock` 分配的地址后，向指定的 DataNode 流式推送数据。为极致优化网络带宽，采用“首包传元数据，后续包传纯数据”的机制，并由 DataNode 自动向底层推进流水线 (Pipeline) 复制。
...
* **Response (Unary):**
  * `bytes_written` (uint64): 实际接收并成功写入磁盘的总字节数。
